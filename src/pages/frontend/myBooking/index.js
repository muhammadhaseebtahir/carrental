...
// const [isLoading,setIsLoading]=  useState(false)
  // console.log("User Bookings:", getUserBooking);
...
// setIsLoading(true)
      try {
        ...
        setGetUserBooking(data.bookings);
      } catch (error) {
...
const content = <div style={contentStyle} />;
  return (
    ...
    <div className="px-6 md:px-16 lg:px-24 xl:px-32 max-w-6xl text-sm 2xl:px-48 pb-20  ">
      <div className="flex flex-col items-center md:items-start justify-center pt-20 pb-10 ">
        <h1 className="text-3xl font-semibold text-gray-800 dark:text-gray-200">My Bookings</h1>
  ...
  {!isAuthenticated  && getUserBooking.length === 0 ? (
    <p className="text-center text-gray-500">No bookings found.</p>
  ) :  ( isAuthenticated && getUserBooking.length === 0 ? (<Spin tip="Loading" size="large">
        {content}
      </Spin>) :(
    getUserBooking.map((item, i) => (
      ...
      >
        ...
        <div className="md:col-span-1">
          ...
          <p className="font-bold text-gray-900 dark:text-gray-100">{item.car?.brand}</p>
        ...
        <div className="md:col-span-2">
          <div className="flex items-center gap-2">
            <p className="px-3 py-1.5 bg-gray-200 rounded">Booking #{i + 1}</p>
            <p
