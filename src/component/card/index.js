...
export default function Card({ card }) {
  ...
  return (
    <div
      onClick={() => {
        navigate(`/product-details/${card._id}`, { state: { card } });
        window.scrollTo(0, 0);
      }}
    ...
    >
      <div className="h-48 w-80 overflow-hidden relative">
        <img
          src={card.image.url}
          alt="heroCar"
        ...
        <p className="absolute bottom-4  right-4 px-3 py-2 rounded-xl font-bold z-10  bg-black/80 backdrop-blur-smf text-white">
          ${card.pricePerDay}/ <span className="text-gray-300">day</span>
        </p>
      </div>
      <div className="card-body flex flex-col gap-1 p-4">
        <div className="text-gray-800 dark:text-gray-300">
          <p className="car-title font-bold text-lg ">{card.brand}</p>
          <p className="text-sm">
        ...
        <div className="flex  gap-2 text-gray-500 ">
          <p className="w-2/3">
            {" "}
            <UsergroupDeleteOutlined className="pr-2" /> {card.seating_capacity}{" "}
            Seats
          </p>
          <p className="w-2/3 ">
            <i className="ri-gas-station-line pr-2"></i> {card.fuel_type}
          </p>
        </div>
        <div className="flex  gap-2 text-gray-500 ">
          <p className="w-2/3">
            <i className="ri-car-line pr-2"></i> {card.transmission}
          </p>
          <p className="w-2/3">
            {" "}
